...
co2_chilled=CO2 %>% group_by(Type,Treatment,conc) %>%
  summarize(mean_uptake=mean(uptake),sd_uptake=sd(uptake),mean_plus_sd=mean_uptake+sd_uptake) %>% filtestats::r(Treatment=="chilled",conc<=350)
```
...
sorted_co2_chilled=CO2 %>%
	stats::filter(Treatment=="chilled") %>%
	arrange(uptake)
...
sorted_co2_non_chilled=CO2 %>%
	stats::filter(Treatment=="nonchilled") %>%
	arrange(uptake)
