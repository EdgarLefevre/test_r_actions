...
nosym_db <- bm_sp %>% dpylr::filter(is.na(external_gene_name) | external_gene_name == "")
...
nosymbol.here <- rowdatadf %>%
  dpylr::filter(is.na(external_gene_name) | external_gene_name == "")

ncbi.here <- opn %>% dpylr::filter(ENSEMBL %in% nosymbol.here$ensembl_gene_id)
print(dim(ncbi.here))

K <- rowdatadf
if (shortname == "human") {
  tmp <- left_join(ncbi.here, nosym_db[, c(
    equi_id,
    "ensembl_gene_id"
  )],
...
symat <- rawmat
rownames(symat) <- rowdatadf$symbol_unique
